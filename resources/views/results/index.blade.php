...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="space-y-3">
        ...
        <div class="bg-[#1e293b] rounded-2xl border border-white/5 overflow-hidden">
            ...
            <div class="px-4 pb-4">
                @php
                    $sportType = $game->sport_config['type'] ?? null;
                ...
                @if($isRunning)
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="flex items-center justify-between bg-white/5 rounded-xl py-3 px-4 {{ $isDq ? 'border border-red-500/30' : '' }}">
                            <div class="flex items-center gap-3">
                                ...
                                @elseif($i === 3)
                                    <span class="text-lg">🥉</span>
                                @else
                                    <span class="text-sm text-slate-500 font-bold w-6">4th</span>
                                @endif
                                <span class="text-sm font-bold {{ $placeTeam ? ($isDq ? 'text-red-400 line-through' : 'text-white') : 'text-slate-500' }}">
                                    {{ $placeTeam?->name ?? '—' }}
                                </span>
                                @if($isDq)
                ...
                <div class="flex items-stretch gap-3">
                    ...
                    <div class="flex-1 flex flex-col items-center bg-white/5 rounded-xl py-3 px-2 {{ $game->winner_id === $game->teamHome?->id ? 'ring-2 ring-blue-500/40' : '' }}">
                        <span class="w-3 h-3 rounded-full mb-2" style="background-color: {{ $game->teamHome?->color_hex ?? '#6b7280' }}"></span>
                    ...
                    <div class="flex-1 flex flex-col items-center bg-white/5 rounded-xl py-3 px-2 {{ $game->winner_id === $game->teamAway?->id ? 'ring-2 ring-blue-500/40' : '' }}">
                        <span class="w-3 h-3 rounded-full mb-2" style="background-color: {{ $game->teamAway?->color_hex ?? '#6b7280' }}"></span>
