...
<div id="live-scoring-app"
     ...
     data-winning-score="{{ $sportConfig['winning_score'] ?? 21 }}"
     data-lead-required="{{ $sportConfig['lead_required'] ?? 2 }}"
    ...
    <div class="bg-[#1e293b] rounded-2xl p-5 border border-white/5 mb-5">
        ...
        {{-- Complete Set / Update Set button (set-based sports only) --}}
        @if($sportType === 'sets')
        <div class="mt-4" id="set-action-container">
            <button type="button" id="set-action-btn" class="hidden w-full py-3 rounded-xl text-sm font-bold transition-colors">
                {{-- Label set by JS: Complete Set or Update Set --}}
            </button>
        </div>
        @endif
    </div>
...
<script>
    ...
    const config = {
        ...
        winningScore: parseInt(app.dataset.winningScore) || 21,
        leadRequired: parseInt(app.dataset.leadRequired) || 2,
    };
    ...
    function setPeriodData(index, home, away) {
        ...
        if (config.sportType === 'sets') {
            ...
            const existing = gameData.sets[index] || {};
            gameData.sets[index] = { ...existing, home, away };
        } else {
    ...
    function recalculateTotals() {
        if (config.sportType === 'sets') {
            ...
            (gameData.sets || []).forEach((s) => {
                ...
                if (home > away) winsHome++;
                else if (away > home) winsAway++;
            });
            scoreHome = winsHome;
            scoreAway = winsAway;
        } else if (config.sportType === 'quarters' || config.sportType === 'halves') {
    ...
    // Update Complete Set / Update Set button visibility and label
    function updateSetActionButton() {
        const btn = document.getElementById('set-action-btn');
        const container = document.getElementById('set-action-container');
        if (!btn || !container || config.sportType !== 'sets') return;

        const currentSet = (gameData.sets || [])[activePeriodIndex] || {};
        const home = Number(currentSet.home) || 0;
        const away = Number(currentSet.away) || 0;
        const hasScores = home > 0 || away > 0;
        const isComplete = !!currentSet.complete;

        if (isComplete) {
            btn.textContent = 'Update Set';
            btn.className = 'w-full py-3 rounded-xl text-sm font-bold transition-colors bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30 active:scale-95';
            btn.classList.remove('hidden');
        } else if (hasScores) {
            btn.textContent = 'Complete Set';
            btn.className = 'w-full py-3 rounded-xl text-sm font-bold transition-colors bg-green-500/20 border border-green-500/40 text-green-400 hover:bg-green-500/30 active:scale-95';
            btn.classList.remove('hidden');
        } else {
    ...
    // Handle Complete Set / Update Set button click
    function handleSetAction() {
        if (config.sportType !== 'sets') return;
        ...
        if (isComplete) {
            // Update Set: recalculate totals from current scores
            recalculateTotals();
            renderPeriodSummary();
            updateSetActionButton();
            scheduleSave();
        } else {
            // Complete Set: mark set complete, add to total, advance to next
            ...
            if (activePeriodIndex < maxPeriods - 1) {
                showToast('set-complete', 2000);
    ...
    const setActionBtn = document.getElementById('set-action-btn');
    if (setActionBtn) {
        setActionBtn.addEventListener('click', handleSetAction);
    }
    ...
    document.querySelectorAll('.status-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            recalculateTotals(); // Recalc when status changes (e.g. completed includes current set)
            scheduleSave();
        });
    });
    ...
    // Override showToast to handle set-complete
    const originalShowToast = showToast;
    window.showToast = function(type, duration) {
        if (type === 'set-complete') {
            if (indicatorTimeout) clearTimeout(indicatorTimeout);
            toastIcon.innerHTML = '';
            toastText.textContent = 'Set complete! Moving to next set...';
            toastInner.className = 'rounded-xl px-3 py-2 text-xs font-medium flex items-center gap-2 border bg-blue-500/20 border-blue-500/40 text-blue-300';
            toastIndicator.classList.remove('hidden');
            indicatorTimeout = setTimeout(() => {
                toastIndicator.classList.add('hidden');
            }, duration);
        } else {
            originalShowToast(type, duration);
        }
    };
